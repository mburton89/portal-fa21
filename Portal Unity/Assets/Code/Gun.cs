...
public class Gun : MonoBehaviour
{
    ...
    void ShootPortal(bool isBlue)
    {
        ...
        if (Physics.Raycast(FiringPoint.position, transform.TransformDirection(Vector3.forward), out raycastHit, Mathf.Infinity) && raycastHit.collider.gameObject.tag == "CanPortal")
        {
            if (isBlue)
            {
